...
fee_A = 4000
fee_B = 2000
fee_receiver_settle = 2000
fee_sender_settle = 1000
...
#   @logger.info("before funding:#{capacity_A}")
#   id, @monitor_A, @monitor_B = tests.create_udt_channel(100, 100, fee_A, fee_B, fee_receiver_settle)
...
#   @logger.info("after funding:#{capacity_A_after_funding}")
...
#   tests.closing_A_B(id, fee_sender_settle)
...
#   @logger.info("after payment:#{capacity_A_after_payment1}")
...
#   tests.assert_equal(fee_A + fee_sender_settle, capacity_A - capacity_A_after_payment1, "A'capacity after payment is wrong.")
#   tests.assert_equal(fee_B + fee_receiver_settle, capacity_B - capacity_B_after_payment1, "B'capacity after payment is wrong.")
# rescue => e
#   puts e
#   raise e
# ensure
...
# begin
#   id, @monitor_A, @monitor_B = tests.create_ckb_channel(1000, 1000, fee_A, fee_B, fee_receiver_settle)
...
#   tests.closing_B_A(id, fee_sender_settle)
#   amount = amount_B_A - amount_A_B
...
#   tests.assert_equal(-amount * 10 ** 8 + fee_A + fee_receiver_settle, balance_A - balance_A_after_payment1, "A'balance after payment is wrong.")
#   tests.assert_equal(amount * 10 ** 8 + fee_B + fee_sender_settle, balance_B - balance_B_after_payment1, "B'balance after payment is wrong.")
# rescue Exception => e
#   raise e
# ensure
#   tests.close_all_thread(@monitor_A, @monitor_B, @db)
